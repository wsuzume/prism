...
func (c *AesGcmJwtConfig) Normalize() (*AesGcmJwtConfig, error) {
	...
	// デフォルト補完
	if n.SecretCookieName == "" { n.SecretCookieName = DefaultSecretCookieName }
	if n.AccessCookieName == "" { n.AccessCookieName = DefaultAccessCookieName }
	if n.SubmitHeaderName == "" { n.SubmitHeaderName = DefaultSubmitHeaderName }

	if n.SecretHeaderName == "" { n.SecretHeaderName = DefaultSecretHeaderName }
	if n.AccessHeaderName == "" { n.AccessHeaderName = DefaultAccessHeaderName }
	if n.PublicHeaderName == "" { n.PublicHeaderName = DefaultPublicHeaderName }
	if n.NotifyHeaderName == "" { n.NotifyHeaderName = DefaultNotifyHeaderName }
